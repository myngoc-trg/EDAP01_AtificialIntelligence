...
class Dashboard:
    ...
    def __init__(self, ROOM_HEIGHT, ROOM_WIDTH, UniformF, T_MINUS_K):
        self.slider_h = widgets.IntSlider(min=ROOM_HEIGHT, max=10, step=1, value=ROOM_HEIGHT)
        self.slider_w = widgets.IntSlider(min=ROOM_WIDTH, max=10, step=1, value=ROOM_WIDTH)
        
        self.slider_h.observe(self.on_slider_change, names='value')
        self.slider_w.observe(self.on_slider_change, names='value')
        
        self.slider_windowL = widgets.IntSlider(min=0, max=10, step=1, value=T_MINUS_K)
        self.slider_windowL.observe(self.on_slider_change, names='value')

        self.slider_f = widgets.IntSlider(min=0, max=1, step=1, value=UniformF)
        self.slider_f.observe(self.on_slider_change, names='value')
        
 
        self.out = widgets.Output(layout=widgets.Layout(height='770px', width='720px', border='solid'))
        ...
        self.input_widgets = widgets.HBox([widgets.VBox([widgets.Text('Height'), self.slider_h]), widgets.VBox([widgets.Text('Width'), self.slider_w])])
        self.input_widgets2 = widgets.HBox([widgets.VBox([widgets.Text('SensorModel 0=NUF, 1=UF'), self.slider_f]), widgets.VBox([widgets.Text('Smoothing window length, 0 = filtering'), self.slider_windowL])])
        ...
        self.db = widgets.VBox([self.input_widgets, self.input_widgets2, self.middle, self.animation])

        # setup of the initial simulation
        self.room = StateModel(self.slider_h.value, self.slider_w.value)
        self.model = Localizer(self.room, self.slider_f.value, self.slider_windowL.value)
    ...
    def update_grid_one_step(self, plotting):
        ...
        try:
            with self.shellout:
                ...
                outputString=""
                if sensed:
                    outputString+="true pose = <" + str(trueR) + "," + str(trueC) + ", " + str(trueH) + ">, sensed position = <" + str(sensedR) + "," + str(sensedC) + ">, guessed position = <" + str(guessedR) + "," + str(guessedC) + ">"
                else:
                    outputString+="true pose = <" + str(trueR) + "," + str(trueC) + ", " + str(trueH) + ">, sensed nothing"
                ...
                self.nbr_of_moves +=1 
                outputString+="\n nbr of moves: " + str(self.nbr_of_moves) + ", avg error: " + str(self.total_error/self.nbr_of_moves) + ", nbr correct guesses: " + str(self.correct_guesses)

                if plot == None:
                    pass
                else:
                    self.update_plt(plot, plotting, outputString)
    ...
    def update_plt(self, plt, plotting, printout=""):
        with self.out:
